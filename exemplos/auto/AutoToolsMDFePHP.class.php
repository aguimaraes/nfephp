...
require_once('/var/www/nfephp/mdfe_homologacao/libs/MDFeNFePHP.class.php');   
//carrega a classe de conversões de txt para xml e vice-versa
require_once('/var/www/nfephp/mdfe_homologacao/libs/ConvertMDFePHP.class.php');
//carrega a classe de impressao da DANFE
//****require_once('DanfeNFePHP6.class.php');
//carrega a classe de envio de emails
//require_once('MailNFePHP.class.php');

class AutoToolsMDFePHP extends MDFeNFePHP {
     ...
     * @return boolean true sucesso false Erro
     */
    public function autoTXTtoXML($tipEmiss){
        echo "\n".'varre pasta "entradas" a procura de MDFes em txt'."\n";
        
        $aName = $this->listDir($this->entDir,'*-mdfe.txt',false);
        echo $this->entDir."\n";
        ...
        if ( $totTXT > 0){
            for ( $x=0; $x < $totTXT; $x++ ) {
                ...
                echo "instancia a classe de conversão\n";
                $oXML = new ConvertMDFePHP();
                echo "converte o arquivo $filename, ".$this->tpAmb.", $tipEmiss \n";
                $xml = $oXML->MDFetxt2xml($filename, $this->tpAmb, $tipEmiss);
                echo "salvar o xml \n";
                $xmlname = $this->entDir.$oXML->chave.'-mdfe.xml';
     ...
     */
    public function autoEnvMDFe(){
        //estabelece condição inicial do retorno
        $recibo = '';
        //varre a pasta de validadas
        $aName = $this->listDir($this->valDir,'*-mdfe.xml',false);
        //se houver algum arquivo *-mdfe.xml continua, caso contrario sai
        $n = count($aName);
        echo "\n $n numero... \n";
        if ( $n > 0 ) {
            ...
            for ($i = 0 ; $i < $k ; $i++) {
                ...
                if ($ret = $this->sendLot($aNFe,$num,$this->modSOAP)){
                    echo "\n incrementa o numero do lote no controle __ $num __ \n";
    ...
    public function autoProtNFe(){
        ...
        $aName = $this->listDir($this->envDir,'*-mdfe.xml',false);
        //se houver algum arquivo *-nfe.xml continua, caso contrario sai
        if ( count($aName) > 0 ) {
            ...
            foreach ( $aName as $file ) {
                ...
                if ( $aRet['cStat'] > 199 ) {
                    ...
                    rename($nfeFile, $pasta.$idNFe.'-mdfe.xml');
                }//endif
                if ( $aRet['bStat'] ) {
                    ...
                    if ( is_file($protFile) && is_file($nfeFile) ) {
                        ...
                        if ($aRet['cStat'] == 100 || $aRet['cStat'] == 110 ) {
                            ...
                            if ( file_put_contents($pasta.$idNFe.'-mdfe.xml', $procnfe) ) {
                ...
                $aRetorno[$n]['nfepath'] = $pasta.$idNFe.'-mdfe.xml';
                $n++;
            }//endforeach
        } else {
    ...
    */
    public function autoPrintSend($fixpdf=false){
        ...
        $aNApr = $this->listDir($this->aprDir,'*-mdfe.xml',false);
        //se houver algum arquivo *-nfe.xml continua, caso contrario sai
        if ( count($aNApr) > 0 ) {
            ...
            foreach ( $aNApr as $file ) {
                ...
                $danfe = new DamdfeNFePHP($docxml,$this->danfepaper,$this->danfeform,$this->danfelogopath,'I','',$this->danfefont);
                $id = (string) $danfe->montaDAMDFE();
                $pdfName = $id.'.pdf';
  				//carrega a DAMDFE como uma string para gravação no diretorio escolhido
                $pdfFile = (string) $danfe->printDAMDFE($this->pdfDir.$pdfName,'S');
                if ($pdfFile != ''){
                    //grava a DAMDFE como pdf no diretorio
			...
			if ( $this->danfeprinter != '' ) {
                            ...
                            $command = "lp -d $this->damdfeprinter $this->pdfDir$pdfName >> /var/www/mdfe/log_print_h.txt";
							system($command);
                ...
                $ver = trim($infMDFe->getAttribute("versao"));
                $razao = utf8_decode($dest->getElementsByTagName("xNome")->item(0)->nodeValue);
                $numero = str_pad($ide->getElementsByTagName('nMDF')->item(0)->nodeValue, 9, "0", STR_PAD_LEFT);
    ...
    public function autoSignNFe() {
        ...
        $aName = $this->listDir($this->entDir,'*-mdfe.xml',false);
        // se foi retornado algum arquivo
        $totXML = count($aName);
        if ( $totXML > 0){
            for ( $x=0; $x < $totXML; $x++ ) {
                ...
                if ( $nfefile = file_get_contents($filename) ) {
                    ...
                    //mantenha desse jeito mesmo com um unico =
                    //a atribuição da variavel e o teste são simultâneos
                    if ( $signn = $this->signXML($nfefile, 'infMDFe') ) {
    ...
    public function autoValidNFe() {
        ...
        $aName = $this->listDir($this->assDir,'*-mdfe.xml',false);
        // se foi retornado algum arquivo
        $totXML = count($aName);
        echo "\n No. assinadas ".$totXML."\n";
        if ( $totXML > 0 ){
            for ( $x=0; $x < $totXML; $x++ ) {
                ...
                if ( $nfefile = file_get_contents($filename) ) {
                    ...
                    $node = $xmldoc->getElementsByTagName('infMDFe')->item(0);
                    //obtem a versão do layout da NFe
                    $ver = trim($node->getAttribute("versao"));
                    //buscar o nome do scheme
                    $filexsd = $this->listDir($this->xsdDir . $this->mdfeSchemeVer. DIRECTORY_SEPARATOR,'mdfe_v*.xsd',true);
                    if (!$filexsd[0]) {
